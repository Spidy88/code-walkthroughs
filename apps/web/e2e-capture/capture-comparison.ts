/**
 * Verifies Chunk 19: /comparison page renders the three-layer
 * surface against the bundled fixture (an exported function becomes
 * private + a brand-new exported function appears).
 */
...
if (!REPO_PATH) {
  console.error('usage: capture-comparison.ts <codebase-path>');
  process.exit(64);
}
...
async function main(): Promise<void> {
  ...
  await page.getByTestId('project-overview-comparison-link').click();
  await page.waitForURL(/\/comparison$/);
  await page.waitForSelector('[data-testid="comparison-base-source"]');
  await page.evaluate(() => document.fonts.ready);
  await page.waitForTimeout(1000);
  await page.screenshot({ path: `${SHOTS}/comparison-1-setup.png`, fullPage: true });

  await page.getByTestId('comparison-run').click();
  await page.waitForSelector('[data-testid="comparison-contract-list"]');
  await page.waitForTimeout(1000);
  await page.screenshot({ path: `${SHOTS}/comparison-2-results.png`, fullPage: true });
  ...
  if (v) {
    ...
    try {
      renameSync(raw, `${VIDS}/comparison-flow.webm`);
    } catch (err) {
